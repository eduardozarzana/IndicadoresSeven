  ...
  const formSectors: FormSector[] = useMemo(() => {
    return sectorsData.map(sector => ({
      ...
      indicators: sector.indicators.map(indicator => {
        ...
        const originalId = idParts.length > 1 ? idParts.slice(1).join('_') : indicator.id; 
        return {
          id: indicator.id, 
          name: indicator.name,
          originalId: originalId,
          isMandatory: indicator.isMandatory, // Propagate from source data
        };
      }),
    }));
  }, [sectorsData]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (currentSector.indicators.length > 0) {
      for (const indicator of currentSector.indicators) {
        ...
        if (inputValue) { // If there is input
        ...
        } else { // Input is empty or undefined
            if (indicator.isMandatory !== false) { // Default to mandatory (isMandatory undefined or true)
                setFormError(`"${indicator.name.replace('*', '').trim()}" é obrigatório.`);
                return; 
            } else {
                // Optional and empty, send "N/A"
                valorParaEnviar = "N/A";
            }
        }
      ...
      // This check is essentially for making sure the loop ran if indicators exist.
      // The prior logic ensures that if it exits early due to error, formDataList might be incomplete.
      // If all pass, formDataList will have one entry per indicator.
      if (formDataList.length !== currentSector.indicators.length && currentSector.indicators.length > 0) {
         // This state implies an error occurred and loop was broken.
         // The error message would have already been set.
         return;
      }

    } else {
      // Sector has no indicators defined in the form structure.
      // onSubmit([]) will be called, App.tsx handles info message.
    }

    try {
      await onSubmit(formDataList);
    } catch (error) {
      // Error display during submission is handled by App.tsx's formSubmissionStatus
    }
  };
              ...
              {currentSector.indicators.map(indicator => (
                <div key={indicator.id}>
                  <label htmlFor={indicator.id} className="block text-sm font-medium text-gray-700 mb-1">
                    ...
                    {/* Show asterisk only if mandatory (undefined or true) */}
                    {(indicator.isMandatory === undefined || indicator.isMandatory === true) && (
